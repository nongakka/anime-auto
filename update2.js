...
for (const file of files) {
  ...
  data.forEach(m => oldMap.set(m.link, m));
}
...
// 🔥 ถ้าเป็นเรื่องเก่า ให้เช็คเฉพาะตอนแรก
if (movie.episodes.length > 0) {

  const firstEp = epElements[0];

  if (firstEp) {
    const latestLink = normalizeUrl(
      $detail(firstEp).attr("href")
    );

    if (movie.episodes.find(x => x.link === latestLink)) {
      console.log("✔ ไม่มีตอนใหม่ ข้าม:", movie.title);
      continue;
    }
  }
}
